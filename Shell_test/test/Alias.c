#include "Shell.h"

/** Names of functions **/

void print_alias(alias_t *alias);
int shell_by_alias(char **args, char __attribute__((__unused__)) **front);
void set_alias(char *var_name, char *value);

/**
 * shell_by_alias - Built-in command that prints all aliases, specific
 * aliases, or sets an alias.
 * @front: Double pointer to the beginning of args.
 * @args: Array of arguments
 *
 * Return: If an error occurs - -1. Otherwise - 0.
 */
int shell_by_alias(char **args, char __attribute__((__unused__)) **front)
{
	int a, ret = 0;
	alias_t *temp = aliases;
	char *val;

	if (!args[0])
	{
		while (temp)
		{
			print_alias(temp);
			temp = temp->next;
		}
		return (ret);
	}
	for (a = 0; args[a]; a++)
	{
		temp = aliases;
		val = _strchr(args[a], '=');
		if (!val)
		{
			while (temp)
			{
				if (_strcmp(args[a], temp->name) == 0)
				{
					print_alias(temp);
					break;
				}
				temp = temp->next;
			}
			if (!temp)
				ret = create_error(args + a, 1);
		}
		else
			set_alias(args[a], val);
	}
	return (ret);
}

/**
 * set_alias - Will either set an existing alias 'name' with a new value,
 * 'value' or creates a new alias with 'name' and 'value'.
 * @var_name: Name of the alias.
 * @value: Value of the alias. First character is a '='.
 */
void set_alias(char *var_name, char *value)
{
	alias_t *temp = aliases;
	int len, j, k;
	char *new_value;

	*value = '\0';
	value++;
	len = _strlen(value) - _strspn(value, "'\"");
	new_value = malloc(sizeof(char) * (len + 1));
	if (!new_value)
		return;
	for (j = 0, k = 0; value[j]; j++)
	{
		if (value[j] != '\'' && value[j] != '"')
			new_value[k++] = value[j];
	}
	new_value[k] = '\0';
	while (temp)
	{
		if (_strcmp(var_name, temp->name) == 0)
		{
			free(temp->value);
			temp->value = new_value;
			break;
		}
		temp = temp->next;
	}
	if (!temp)
		add_alias_end(&aliases, var_name, new_value);
}

/**
 * set_alias - Set an existing alias 'name' with a new value,
 * or creates a new alias with 'name' and 'value'.
 * @value: The value of the alias. First character is a '='.
 * @var_name: The name ame of the alias
 *
 */

void set_alias(char *var_name, char *value)
{
	alias_t *temp = aliases;
	int len;
	int a;
	int b;
	char *new_valu;

	*value = '\0';
	value++;
	len = _strlen(value) - _strspn(value, "'\"");
	new_valu = malloc(sizeof(char) * (len + 1));
	if (!new_valu)
		return;
	for (a = 0, b = 0; value[a]; a++)
	{
		if (value[a] != '\'' && value[a] != '"')
			new_valu[b++] = value[a];
	}
	new_valu[b] = '\0';
	while (temp)
	{
		if (_strcmp(var_name, temp->name) == 0)
		{
			free(temp->value);
			temp->value = new_valu;
			break;
		}
		temp = temp->next;
	}
	if (!temp)
		add_alias_end(&aliases, var_name, new_valu);
}

/**
 * print_alias - Function that prints the alias in the format name='value'.
 * @alias: The pointer to an alias.
 */

void print_alias(alias_t *alias)
{
	char *alias_string;
	int len = _strlen(alias->name) + _strlen(alias->value) + 4;

	alias_string = malloc(sizeof(char) * (len + 1));
	if (!alias_string)
		return;
	_strcpy(alias_string, alias->name);
	_strcat(alias_string, "='");
	_strcat(alias_string, alias->value);
	_strcat(alias_string, "'\n");

	write(STDOUT_FILENO, alias_string, len);
	free(alias_string);
}

/**
 * replace_aliases - replaces any matching alias with their value.
 * @args: 2D pointer to the arguments.
 *
 * Return: 2D pointer to the arguments.
 */

char **replace_aliases(char **args)
{
	alias_t *temp;
	int i;
	char *new_value;

	if (_strcmp(args[0], "alias") == 0)
		return (args);
	for (i = 0; args[i]; i++)
	{
		temp = aliases;
		while (temp)
		{
			if (_strcmp(args[i], temp->name) == 0)
			{
				new_value = malloc(sizeof(char) * (_strlen(temp->value) + 1));
				if (!new_value)
				{
					free_args(args, args);
					return (NULL);
				}
				_strcpy(new_value, temp->value);
				free(args[i]);
				args[i] = new_value;
				i--;
				break;
			}
			temp = temp->next;
		}
	}

	return (args);
}
